"""CustomTkinter GUI to monitor incoming AX.25 traffic on the Mobilinkd
TNC live.

Opens its own direct RFCOMM connection to the TNC (via rfcomm_connect.py,
same as kiss_bridge.py) rather than tapping kiss_bridge.py's KISS pty:
the TNC's Bluetooth SPP link only accepts one client at a time, and a
pty slave has no "broadcast to every reader" semantics -- a second
reader there would silently steal bytes out from under
kissattach/Pat rather than seeing a copy of them. So this is its own
one-shot connection, mutually exclusive with the KISS bridge running --
stop it first.

Each complete KISS frame received is decoded with kiss.py (framing) and
ax25.py (source/dest callsigns, digipeater path, frame type, PID) and
shown as a row in a live table; selecting a row shows the full header
and hex/ASCII payload dump below it.
...
# ---------- Persistent TNC session (runs in a background asyncio loop) ----------

class AsyncLoop(threading.Thread):
    """A background thread running its own asyncio event loop, so the
    Tkinter main loop is never blocked by TNC I/O. Submit coroutines
...
class MonitorSession:
    """Holds one RFCOMM connection to the TNC open, decoding incoming
    KISS frames and handing the raw AX.25 payloads to the GUI thread via
    a thread-safe queue.
    ...
    async def connect(self, addr: str) -> None:
        ...
        # connect_rfcomm() does blocking socket I/O (channel probing) --
        # run it off the event loop so it doesn't stall anything else
        # scheduled on it.
        sock = await loop.run_in_executor(None, connect_rfcomm, addr, None)
    ...
    async def _read_loop(self) -> None:
        ...
        try:
            while True:
                ...
                for kiss_frame in decoder.feed(data):
                    if not kiss_frame:
                        continue
                    self.frame_queue.put_nowait((time.time(), bytes(kiss_frame[1:])))
        except asyncio.CancelledError:
            raise
        except Exception as e:
            self.disconnect_queue.put_nowait(e)
...
class ChannelMonitorGUI(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Mobilinkd TNC Channel Monitor")
        ...
        self._remembered = load_remembered_device() or (
            (DEFAULT_DEVICE_UUID, DEFAULT_DEVICE_UUID) if DEFAULT_DEVICE_UUID else ("", "")
        )
    ...
    def _scan_done(self, devices, error):
        ...
        if self._remembered[0] and self._remembered[0] not in self._scan_map:
    ...
    def _on_connect(self):
        ...
        if pgrep(r"kiss_bridge\.py"):
            self._log(
                ...
                " the TNC only allows one Bluetooth connection at a time.\n"
            )
            return
    ...
    def _on_connected(self):
        ...
        self._log("Connected. Monitoring for AX.25 traffic.\n")

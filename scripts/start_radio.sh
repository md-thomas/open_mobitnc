#!/bin/bash
# Start the Mobilinkd TNC Bluetooth<->KISS bridge and attach it to the
# kernel AX.25 stack, without needing multiple terminals or `screen`.
#
# Usage:
#   scripts/start_radio.sh [device_uuid]
#
# device_uuid defaults to tnc_config.py's DEFAULT_DEVICE_UUID (override
# via the MOBITNC_ADDR env var) if omitted -- run scripts/scan_radio.sh
# first if you don't already know the TNC's address.
#
...
#   kill -INT "$(cat ~/.cache/mobitnc-bridge.pid)"
# (plain SIGINT, not -9 -- an abrupt kill can leave ax0 stuck at the
# kernel level). Stop kissattach with `sudo pkill -f kissattach`, or just
# run stop_radio.sh.
#
# If the SUDO_PASS env var is set, it's piped to `sudo -S` instead of
# prompting on the terminal. Not persisted anywhere; leave it unset for
...
# back to tnc_config.py's DEFAULT_DEVICE_UUID itself -- one source of
# truth for the address instead of duplicating it here.
DEVICE_UUID="${1:-}"
PTY_PATH="$HOME/.cache/mobitnc-kisstnc"
LOG_PATH="$HOME/.cache/mobitnc-bridge.log"
PID_PATH="$HOME/.cache/mobitnc-bridge.pid"

#!/bin/bash
# Stop the Mobilinkd TNC Bluetooth<->KISS bridge and kissattach started
# by scripts/start_radio.sh (or run manually).
#
# Usage:
...
# --- kiss_bridge.py: SIGINT, never -9 (an abrupt kill can leave ax0
# stuck at the kernel level) ---
BRIDGE_PID=""
...
if [ -n "$BRIDGE_PID" ] && kill -0 "$BRIDGE_PID" 2>/dev/null; then
    ...
    if kill -0 "$BRIDGE_PID" 2>/dev/null; then
        ...
        echo "Check ~/.cache/mobitnc-bridge.log before considering kill -9."
    else
...
# --- ax0 sanity check: orphaned interface would leave a stuck kernel
# device with no owning process ---
if ip link show ax0 >/dev/null 2>&1 && ! pgrep -af kissattach >/dev/null 2>&1; then
    ...
    echo "Confirm with: sudo rmmod mkiss"

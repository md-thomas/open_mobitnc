#!/bin/bash
# One-time setup: let the Mobilinkd TNC scripts (start_radio.sh,
# stop_radio.sh) attach/detach the KISS TNC without a password prompt
# every time, by installing a narrowly-scoped NOPASSWD sudoers rule for
...
# more general line already there (e.g. a plain `ALL=(ALL:ALL) ALL`). A
# dedicated sudoers.d file is guaranteed to be evaluated last and win.
#
# The SUDO_PASS env var in start_radio.sh/stop_radio.sh still works
# either way -- this just makes it optional instead of required.
#
# Usage:
...
RULES_FILE="/etc/sudoers.d/mobitnc-radio"
TMP_FILE="$(mktemp)"
trap 'rm -f "$TMP_FILE"' EXIT

cat > "$TMP_FILE" <<EOF
# Managed by open_mobitnc/scripts/install_sudo_rules.sh -- do not
# hand-edit, re-run that script instead. Lets $TARGET_USER attach/detach
# the Mobilinkd TNC's KISS port without a password prompt. See that
# script for why this is a separate sudoers.d file and why pkill (not
# plain kill) is used.
